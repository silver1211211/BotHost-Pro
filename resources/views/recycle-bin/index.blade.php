...
<div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-[#71717A]">Restore deleted bots or permanently remove old items. Bots are kept for {{ $retentionDays }} days.</p>
        </div>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        @foreach([
            ['Deleted Items', $deletedCount,  '#EF4444', 'M20.25 7.5l-.625 10.632A2.25 2.25 0 0 1 17.378 20.25H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0-3-3m3 3 3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z'],
            ...
            ['Restorable',    $deletedCount,  '#22C55E', 'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99'],
    ...
    <div class="rounded-2xl border border-[#27213D] bg-[#0F0D1A] p-5">
        @if ($bots->isEmpty())
            <div class="flex flex-col items-center gap-4 rounded-xl border border-[#1B172B] bg-[#0B0918] py-16 text-center">
                ...
                <div>
                    <p class="text-base font-black text-[#71717A]">Recycle Bin is empty</p>
                    ...
                    <div class="rounded-xl border border-[#1B172B] bg-[#0B0918] p-4">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-[#71717A]">{{ $bot->telegram_username ? '@'.$bot->telegram_username : 'No Telegram username' }}</p>
                            ...
                            <div class="flex shrink-0 flex-wrap gap-2">
                                ...
                                <form method="POST" action="{{ route('recycle-bin.bots.force-delete', $bot->id) }}">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="rounded-2xl border border-[#27213D] bg-[#0F0D1A] p-5">
        <p class="mb-4 text-[10px] font-black uppercase tracking-widest text-[#71717A]">How the Recycle Bin Works</p>
