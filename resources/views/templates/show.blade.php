<x-dashboard-layout title="{{ $template->name }}">
        ...
        <div class="rounded-2xl border border-[#27213D] bg-[#0F0D1A] p-5">
            <div class="relative mb-5 w-full overflow-hidden rounded-xl" style="aspect-ratio:16/9">
                @if($template->thumbnail_url)
                    ...
                    <div class="absolute inset-0 items-center justify-center bg-[#11101C] text-sm text-[#71717A]" style="display:none">No image</div>
                @else
                    <div class="flex h-full w-full items-center justify-center border border-[#27213D] bg-[#11101C] text-sm text-[#71717A]">No image</div>
                @endif
            </div>
            ...
            <div class="mt-3 flex flex-wrap gap-2 text-xs text-[#71717A]"><span>{{ $template->category ?: 'General' }}</span><span>{{ ucfirst($template->level) }}</span><span>{{ $template->commands_count }} commands</span><span>{{ $template->import_count }} imports</span>@if($template->includedPlanLabel())<span>{{ $template->includedPlanLabel() }}</span>@endif</div>
    ...
    View Demo Bot
            ...
            @if($template->tags)<div class="mt-3 flex flex-wrap gap-1">@foreach($template->tags as $tag)<span class="rounded border border-[#27213D] px-2 py-0.5 text-[10px] text-[#71717A]">{{ $tag }}</span>@endforeach</div>@endif
        </div>
            ...
            @elseif($template->isPaid() && ! $purchased && ! auth()->user()->isAdmin())
                @if($pendingInvoice)
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-3">
                        ...
                        <span class="text-xs text-[#71717A]">Invoice status: {{ ucfirst($pendingInvoice->status) }}</span>
                    </div>
                @else
                    ...
                    <form method="POST" action="{{ route('dashboard.templates.crypto-invoice', $template) }}" class="mt-3">
                        ...
                        <p class="mb-3 text-xs text-[#71717A]">You will select your payment network on the next step.</p>
                ...
                <form x-data="{ open: false, val: '{{ $bots->first()?->id }}', urls: @js($bots->mapWithKeys(fn($b) => [$b->id => route('bots.templates.import', [$b, $template])])->all()), labels: @js($bots->pluck('name', 'id')->all()), get label() { return this.labels[this.val] || 'Select bot' } }" :action="val && urls[val] ? urls[val] : '#'" method="POST" class="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
                    ...
                    <div class="relative" @click.away="open = false">
                        ...
                        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1"
                            ...
                            @empty
                            <div class="px-4 py-3 text-sm text-[#71717A]">No bots available</div>
                            @endforelse
                        </div>
                    </div>
