<x-admin-layout title="Payment Settings" subtitle="OxaPay White Label configuration">
    <div class="space-y-5">
        ...
        <div class="rounded-2xl border border-[#27213D] bg-[#0F0D1A] p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-[#94A3B8]">Configure OxaPay White Label payments for template purchases and plan upgrades.</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.deposits.settings.update') }}" class="mt-5 space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    @if($maskedApiKey)
                        <p class="text-xs text-[#94A3B8]">Saved key: <span class="font-mono text-[#A1A1AA]">{{ $maskedApiKey }}</span></p>
                    @endif
                    <p class="text-[11px] text-[#94A3B8]">Stored encrypted. Never displayed in full.</p>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="text-[11px] text-[#94A3B8]">Min 15, max 2880 (48 hours). Default 60.</p>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input name="under_paid_coverage" type="number" min="0" max="100" step="0.01"
                        ...
                        class="w-full rounded-xl border border-[#27213D] bg-[#11101C] px-3.5 py-2.5 text-sm text-white placeholder-[#71717A] outline-none transition focus:border-[#8B5CF6]/60 focus:ring-1 focus:ring-[#8B5CF6]/20">
                    <p class="text-[11px] text-[#94A3B8]">Percentage of underpayment OxaPay will accept as paid.</p>
                </div>
                ...
                <div class="space-y-2 rounded-xl border border-[#27213D] bg-[#11101C] p-4">
                    ...
                    <label class="flex items-start gap-3 cursor-pointer py-1">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-[#94A3B8]">{{ $desc }}</p>
                        </div>
                    </label>
                    @endforeach
                </div>
        ...
        <div class="rounded-2xl border border-[#27213D] bg-[#0F0D1A] p-5">
            ...
            <p class="mt-1 text-xs text-[#94A3B8]">Public callback base: <span class="font-mono text-[#A1A1AA] break-all">{{ $publicCallbackBaseUrl }}</span></p>

            <div class="mt-4 space-y-4">
                <div>
                    <p class="text-xs font-bold uppercase text-[#94A3B8]">OxaPay Webhook URL</p>
                    <p class="text-xs text-[#94A3B8] mt-0.5 mb-1">Enter this URL in your OxaPay merchant dashboard as the callback URL.</p>
                ...
                <div>
                    <p class="text-xs font-bold uppercase text-[#94A3B8]">Return URL base (local)</p>
                    <p class="text-xs text-[#94A3B8] mt-0.5 mb-1">Used for payment return redirects. Uses APP_URL (local browser navigation).</p>
            ...
            <p class="mt-3 text-xs text-[#94A3B8]">Use the generic webhook URL above in your OxaPay dashboard. It handles both template purchases and plan upgrades.</p>
        </div>
    </div>
</x-admin-layout>
